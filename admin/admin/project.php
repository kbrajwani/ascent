...
require 'class.php';
$obj=new database();
$res=$obj->getallproject();
?>
<?php
while($row=$res->fetch_assoc())
{
$user=$obj->getuserbyid($row["fk_uid"]);
$ruser=$user->fetch_assoc();
  echo'                                  <tr class="gradeX">
                                        <th>
                                        <input type="checkbox" name="chk[]" value="'.$row["job_id"].'"> |
                                        <a href="projectdelete.php?id='.$row["job_id"].'"><span class="glyphicon glyphicon-trash"></span></a> |
                                        <a data-toggle="modal" data-target="#update'.$row["job_id"].'"><span class="glyphicon glyphicon-edit"></span></a>
...
$res1=$obj->getallproject();
while($row1=$res1->fetch_assoc())
{
    echo '
    <div class="modal fade" style="height:700px;margin-top:10%" id="update'.$row1["job_id"].'" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-header">
       ...
       <span itemprop="name"><h3>update project '.$row1["title"].'</h3> </span>
    </div>
    <div class="modal-body">
    <form class="form_modal_style" method="post" action="projectupdate.php?id='.$row1["job_id"].'" name="answer" enctype="multipart/form-data">
         <div class="wp-editor-container" class="cols-md-8">
         <table class="table">
         
         <tr><td>title<td><input type="text" name="txttitle" placeholder="'.$row1["title"].'" />
         
        <tr><td>category<td> <input type="text" name="txtcategory" placeholder="'.$row1["category"].'" />
        <tr><td>experience<td> <input type="text" name="txtexperience" placeholder="'.$row1["job_experience"].'" />
        <tr><td>budget<td> <input type="text" name="txtbudget" placeholder="'.$row1["budget"].'" />
        <tr><td>details<td> <input type="text" name="txtdetail" placeholder="'.$row1["extra_details"].'" />
        <tr><td>dead line<td> <input type="text" name="txtdead_line" placeholder="'.$row1["dead_line"].'" />
                    ...
                    <div class="row submit-wrapper">
                        ...
                        <input type="submit" name="btinsert" value="update"/>   
                        </div>
                    </div>
    </form>
...
?>
            ...
            <!-- /.container-fluid -->
            <footer class="footer text-center"> 2017 &copy; Ascent Admin brought to you by Assentdesigner.in </footer>
        </div>
        <!-- /#page-wrapper -->
    </div>
    <!-- /#wrapper -->
